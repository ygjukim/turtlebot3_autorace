...
int DetectLaneNodelet::maskWhiteLaneFromHSV(const Mat& hsv_img, Mat& mask, vector<Point>& locations)
{
    ...
    if (fraction_num > 0 && !is_calibration_mode) {
        ...
        else if (fraction_num < 5000) {     // need to check!
            if (lightness_white_l > 50)  lightness_white_l -= 5;
        }
    }

    // Search the start index of non-zero rows
    int nonzero_row_start = 0;
    int row, col;
    if (fraction_num > 0) {
        for (row=0; row<mask.rows; row++) {
            ...
            if (col < mask.cols)  break;
        }
        nonzero_row_start = row;
    }

    // Publish lane-masked image
...
int DetectLaneNodelet::maskYellowLaneFromHSV(const Mat& hsv_img, Mat& mask, vector<Point>& locations)
{
    ...
    if (fraction_num > 0 && !is_calibration_mode) {
        if (fraction_num > 35000) {
            ...
            if (lightness_yellow_l > 90)  lightness_yellow_l -= 20;     // need to check!
        }
    }
...
int DetectLaneNodelet::sliding_window(const Mat& image, const vector<Point>& nonzero_pts, 
                        int nonzero_row_start, int left_or_right, vector<double>& lane_fit)
{
    int width = image.cols;         // image width
    int height = image.rows;        // image height

    // Create output image for sliding window
    Mat out_image;
    if (is_calibration_mode) {
        cvtColor(image, out_image, CV_GRAY2BGR);
    }

    // Detemine center x-coordinate for starting window
    ...
    // Initialize variable related on sliding windows
    int nwindows = 20;
...
//  int minpixels = 50;
    ...
    int win_half_height = window_height / 2;
    int win_half_width = window_width / 2;
    int win_x = lane_base;
    int prev_win_x = lane_base;
    int win_y = height - win_half_height;

    // Step through the windows one by one
    while ((win_x >= 0 && win_x < width) && (win_y >= 0)) {
        // Identify the nonzero pixels in x and y within the window
        for (int k=0; k<nonzero_pts.size(); k++) {
            int x = nonzero_pts[k].x;
            int y = nonzero_pts[k].y;
            if ((abs(y - win_y) < win_half_height) && 
                    (abs(x - win_x) < win_half_width)) 
            {
        ...
        if (is_calibration_mode) {
            ...
            rectangle(out_image, Point(win_x - win_half_width, win_y - win_half_height), 
                    Point(win_x + win_half_width, win_y + win_half_height), 
                    Scalar(0, 255, 0), 2);
        }

        // Update win_x & win_y and reset related variables
        if (x_count > minpixels) {
            // If you found > minpix pixels, recenter next window on their mean position
            // get next win_x
            prev_win_x = win_x;
            win_x = acc_x_pos / x_count;
            // get next win_y
            if (win_y > nonzero_row_start)
                win_y -= (window_height - 1);
        }
        else {
            // get next win_x
            if (win_x > prev_win_x)  win_x += (window_width - 1);
            else win_x -= (window_width - 1);
            // get next win_y ==> does not change win_y
        }
